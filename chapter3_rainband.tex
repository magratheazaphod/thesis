...
where $\rho(k)$ is the autocorrelation function of rainband presence with lag $k$ \citep{VonStorch1999}. We calculate $\tau$ using m=10 days. The yearly mean decorrelation timescale of rainband frequency is found to be $\tau = 1.81$ after removing the seasonal cycle. This value is used to calculate significance of changes in Figure 3b. The standard deviation and $p$-values of rainband frequency changes in Tables~{tab:t35} and~\ref{tab:t37} use seasonal values of $\tau$. Table~{tab:t311} shows the temporal autocorrelation of other alternative tested metrics of China rainfall. $ 
tau$ is further used to inform the selection of block length in the moving blocks bootstrap, as described below.

%% BOOTSTRAPPING
\subsection{Significance of Changes: Bootstrapping Algorithms}

	The standard deviation and significance of changes in rainband frequency are calculated analytically. However, the distribution of rainband latitude and intensity during a given season does not necessarily follow a normal distribution. Therefore, in the estimation of statistical significance we are required to employ non-parametric tests. In this chapter, estimations of significance were obtained using bootstrapping with and without replacement (the latter also known as a permutation method), well-established techniques that allow the estimation of relevant quantities by constructing synthetic distributions \citep{Good2005}. We use bootstrapping with replacement to calculate the standard deviation of their means (Tables~\ref{tab:t34}-~\ref{tab:t38} respectively). Both bootstrapping with replacement and a permutation test produce similar results; $p$-values shown are from permutation testing. We focus on changes in front attributes between 1951-1979 and 1980-2007 (Tables~\ref{tab:t35} and~\ref{tab:t36}), and also repeat our methodology for 1979-1993 versus 1994-2007, since other authors have found a significant shift between these sets of years.
	
	Weather systems and rainbands may persist over multiple days. It is possible that we can have multiple observations of what is in reality a single anomalous event, and the permutation test above may therefore exaggerate significance. In such circumstances, we use a \textit{moving blocks bootstrap} test, described for instance in \citep{Singh2014}, wherein data are randomly drawn in continuous blocks of length $n$ that preserve the time structure of the original data set. We use the autocorrelation time scale $\tau$ of each statistic to select block length. For instance, the autocorrelation of daily rainfall in China is $\tau =2-4 \mathrm{\ days}$ and generally less than 3 days. Therefore, the calculation of significance of rainfall changes in Figure~\ref{fig:hov}a uses a moving blocks bootstrap appropriate for autocorrelated data with 1000 iterations and block length of 3 days \citep{Singh2014}. The calculation of significance of change in alternative Meiyu statistics (Table~\ref{tab:t312}) uses the $\tau$ of each statistic in each season (Table~\ref{tab:t311}). In general, the choice of alternative block lengths does not strongly alter estimations of significance.
		
	Not all algorithm outputs of interest are continuous in time. Specifically, if no front is observed on a day, then we cannot report a latitude or intensity. There is no existing convention to our knowledge for handling the case of missing observations in a moving blocks bootstrap, and we instead rely on a regular permutation test to gauge the significance of changes in latitude and intensity of rainbands between time periods (Tables~{tab:t36} and~\ref{tab:t38}). Instead, we rely on alternative methods such as the Anderson-Darling and Kolmogorov-Smirnov tests that test whether the entire distribution of latitude or intensity has shifted between time periods. These two methods are described below.
...
\subsection{Significance of Changes in Distributions of latitude and intensity}
...
The Anderson-Darling test is similarly carried out in R, and a $p$-value returned based on the comparison of the test statistic $A$ to known critical values. The results of the K-S and A-D tests for changes in the distribution of rainband latitude and intensity are shown in Tables~\ref{tab:t313} and~\ref{tab:t314}
